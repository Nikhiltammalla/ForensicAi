...
public class DashboardController implements Initializable {
    ...
    public String save_img(){
            ...
            //Directory 
            fileChooser.setInitialDirectory(new File("C:\\Users\\Nikhil\\Downloads"));
    ...
    public String autoSaveImg() {
        ...
        File file = new File("temp_sketch.png");
        try {
            ImageIO.write(SwingFXUtils.fromFXImage(snapshot, null), "png", file);
            return file.getAbsolutePath();
        } catch (IOException ex) {
            System.out.println("Error saving temp image: " + ex.getMessage());
            ex.printStackTrace();
        }
        return null;
    }
    ...
    private void onCompare(MouseEvent event) {
        String savedPath = autoSaveImg(); //Save image automatically
        if (savedPath == null) {
            return;
        }
        
        try {
            com.mycompany.forensicai_facematch.face_rekognition recognition = 
                    new com.mycompany.forensicai_facematch.face_rekognition(savedPath);
